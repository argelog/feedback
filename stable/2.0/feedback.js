...
// Version 2.0.4
//2017, Mesut Çakır, https://github.com/argelog/feedback
...
(function ($) {
    ...
    $.feedback = function (options) {
        ...
        var settings = $.extend({
            extendPostData: {  },
            ajaxURL: '',
            beforePost: function (post) {
                //$.blockUI();
                return true;
            },
            onSuccess: function (jsonResult, defaultCallback,errorCallback) {
                defaultCallback();
            },
            onError: function (res, defaultCallback) {
                defaultCallback();
            },
            ...
            tpl: {
                description: $("#base_sendfeedback_description").html(),
                highlighter: $("#base_sendfeedback_highlighter").html(),
                overview: $("#base_sendfeedback_overview").html(),
                submitSuccess: $("#base_sendfeedback_submitSuccess").html(),
                submitError: $("#base_sendfeedback_submitErrorr").html()
            },
            onClose: function () { },
            ...
            $(document).on('click', settings.feedbackButton, function () {
                ...
                $(document).on('click', '#feedback-submit', function () {
                    ...
                    if ($('#feedback-note').val().length > 0) {
                        ...
                        if (settings.extendPostData) {
                            post = $.extend({}, post, settings.extendPostData);
                        }
                        var isContinue = true;
                        if (settings.beforePost)
                            isContinue  = settings.beforePost(post);
                        if (!isContinue) {
                            ...
                            $.ajax({
                                ...
                                success: function (jsonResult) {
                                    if (settings.onSuccess) {
                                        settings.onSuccess(jsonResult, function () {
